...
public class ClearWriteEditText extends EditText implements View.OnFocusChangeListener , TextWatcher {
    ...
    public ClearWriteEditText(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_UP) {
            if (getCompoundDrawables()[2] != null) {
                ...
                //增加热区
                boolean isInnerWidth = x > (getWidth() - getTotalPaddingRight()-50) && x < (getWidth() - getPaddingRight()+50);
                boolean isInnerHeight = y > (distance-50) && y <(distance + height+50);
