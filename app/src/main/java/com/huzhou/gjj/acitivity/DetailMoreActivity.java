...
import com.huzhou.gjj.adapter.MyBaseExpandableListAdapter;
...
public class DetailMoreActivity extends BaseAppCompatActivity implements
    ...
    private void getZhiDian() {
        ...
        try {
            ...
            data1.put("dictType", "jdbz");
            JSONObject data2 = new JSONObject();
            data2.put("dictType", "gjhtqywlx");
            JSONObject data3 = new JSONObject();
            data3.put("dictType", "zhlx");
            array.put(data1);
            array.put(data2);
            array.put(data3);
    ...
    private void initDatas(int page, final String str) {
        ...
        try {
            obj.put("grzh", grzh);
            obj.put("jzrqBegin", start);
            obj.put("jzrqEnd", end);
            obj.put("page", page);
            obj.put("rows", 10);
            obj.put("startRowNum", 0);
            jsonData = JsonAnalysis.putJsonBody(obj, "2003").toString();
        } catch (JSONException ignored) {
            pro_dialog.dismiss();
        }
        RequestParams params = new RequestParams(SERVICE_REQUEST_URL);
        params.addBodyParameter("code", "2003");// 添加到请求body体的参数, 只有POST, PUT, PATCH, DELETE请求支持.
        ...
        x.http().post(params, new Callback.CommonCallback<String>() {
                    ...
                    public void onSuccess(String result) {
                        try {
                            ...
                            JSONArray array = new JSONObject(body).getJSONArray("grmxxxList");
                            if (array.length() == 0) {
                            ...
                            for (int j = 0; j < array.length(); j++) {
                                detail = new Gson().fromJson(array.get(j).toString(), Detail.class);
                                list.add(detail);
                            }
                            if ("onLoadMore".equals(str)) {
                                ...
                                pageNum = 1;
                                myAdapter = new MyBaseExpandableListAdapter(x.app(), list, list_zd);
    ...
    private void getEvent(View view) {
        switch (view.getId()) {
            case fab:
                startActivity(new Intent(DetailMoreActivity.this, MainActivity.class));
                finish();
                break;
        }
    }
